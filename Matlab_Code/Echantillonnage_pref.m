function [I,V,e,B1,B2] = Echantillonnage_pref(n)
...
% e : erreur standard
S1=0; % S1 correspond a la somme des Z_i : on initialise S1 a 0 car on n'a pas encore fait de simultaion a ce stade
S2=0; % S2 correspond a la somme des (Z_i)^2 : on initialise S2 a 0 car on n'a pas encore fait de simultaion a ce stade
...
for i=1:n
    y=-log(rand()); % On simule une v.a de loi exponentielle de parametre 1 (correspond a Y_i)
    z=(1/sqrt(2*pi))*(exp(sqrt(2*y))-1)/sqrt(2*y); % On simule une v.a de loi Z_1 (correspond a Z_i)
    S1=S1+z; S2=S2+z^2;
    I=S1/i;
    Ivect=[Ivect I];
...
title("Valeur approchee de C en fonction du nombre de simulations par la methode de Monte Carlo avec echantillonnage preferentiel");
